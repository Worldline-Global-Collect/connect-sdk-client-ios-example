...
@interface WCEndViewController ()

@end

@implementation WCEndViewController
...
- (void)viewDidLoad
{
    ...
    label.text = NSLocalizedStringFromTable(@"SuccessLabel", kWCAppLocalizable, nil);
    label.translatesAutoresizingMaskIntoConstraints = NO;
    
    UITextView *textView = [[UITextView alloc] init];
    [container addSubview:textView];
    textView.textAlignment = NSTextAlignmentCenter;
    textView.text = NSLocalizedStringFromTable(@"SuccessText", kWCAppLocalizable, nil);
    ...
    UIButton *button = [self.viewFactory buttonWithType:WCButtonTypePrimary];
    [container addSubview:button];
    NSString *continueButtonTitle = NSLocalizedStringFromTable(@"ContinueButtonTitle", kWCAppLocalizable, nil);
    ...
    _resultButton = [self.viewFactory buttonWithType:WCButtonTypeSecondary];
    [container addSubview:_resultButton];
    NSString *resultButtonTitle = NSLocalizedStringFromTable(@"EncryptedDataResultLabel", kWCAppLocalizable, nil);
...
- (void)resultButtonTapped
{
    if ([_encryptedText isHidden] == YES) {
        NSString *resultButtonTitle = NSLocalizedStringFromTable(@"CopyEncryptedDataLabel", kWCAppLocalizable, nil);
        [_resultButton setTitle:resultButtonTitle forState:UIControlStateNormal];
