//
//  WCPaymentProductSelectionDelegate.m
...
#import <WorldlineConnectSDK/WCBasicPaymentProductGroups.h>
#import <WorldlineConnectExample/WCEndViewController.h>
#import <WorldlineConnectSDK/WCPaymentContext.h>
...
@interface WCPaymentProductsViewControllerTarget ()
...
@property (strong, nonatomic) WCPaymentProduct *applePayPaymentProduct;
...
@implementation WCPaymentProductsViewControllerTarget
...
- (id)init {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:@"-init is not a valid initializer for the class WCPaymentProductsViewControllerTarget"
                                 userInfo:nil];
    return nil;
}
...
- (void)didSelectPaymentItem:(NSObject <WCBasicPaymentItem> *)paymentItem accountOnFile:(WCAccountOnFile *)accountOnFile;
{
    ...
    // payment product in the payment product selection screen, the WCSession
    ...
    if ([paymentItem isKindOfClass:[WCBasicPaymentProduct class]]) {
        [self.session paymentProductWithId:paymentItem.identifier context:self.context success:^(WCPaymentProduct *paymentProduct) {
...
- (void)showPaymentItem:(NSObject <WCPaymentItem> *)paymentItem accountOnFile:(WCAccountOnFile *)accountOnFile {
    WCPaymentProductViewController *paymentProductForm = nil;
    
    if (([paymentItem isKindOfClass:[WCPaymentProductGroup class]] && [paymentItem.identifier isEqualToString:@"cards"]) || ([paymentItem isKindOfClass:[WCPaymentProduct class]] && [((WCPaymentProduct *)paymentItem).paymentMethod isEqualToString:@"card"])) {
        paymentProductForm = [[WCCardProductViewController alloc] init];

    }
    else if ([paymentItem.identifier isEqualToString:kWCBoletoBancarioId]){
        paymentProductForm = [[WCBoletoProductViewController alloc]init];
    }
    else {
        paymentProductForm = [[WCPaymentProductViewController alloc] init];
    }
...
- (void)showApplePaySheetForPaymentProduct:(WCPaymentProduct *)paymentProduct withAvailableNetworks:(WCPaymentProductNetworks *)paymentProductNetworks {
    ...
    NSString *merchantId = [StandardUserDefaults objectForKey:kWCMerchantId];
    if (merchantId == nil) {
        return;
    }
...
- (void)generateSummaryItems {
    ...
    [summaryItems addObject:[PKPaymentSummaryItem summaryItemWithLabel:NSLocalizedStringFromTableInBundle(@"gc.app.general.shoppingCart.subtotal", kWCSDKLocalizable, self.sdkBundle, @"subtotal summary item title") amount:[NSDecimalNumber decimalNumberWithMantissa:subtotal exponent:-2 isNegative:NO]]];
    [summaryItems addObject:[PKPaymentSummaryItem summaryItemWithLabel:NSLocalizedStringFromTableInBundle(@"gc.app.general.shoppingCart.shippingCost", kWCSDKLocalizable, self.sdkBundle, @"shipping cost summary item title") amount:[NSDecimalNumber decimalNumberWithMantissa:shippingCost exponent:-2 isNegative:NO]]];
