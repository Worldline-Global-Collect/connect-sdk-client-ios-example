...
#import "WCJsonDialogViewController.h"
#import "WCStartPaymentParsedJsonData.h"
#import "WCParseJsonTarget.h"
#import "WCViewFactory.h"
#import "WCAppConstants.h"

@interface WCJsonDialogViewController () <UITextViewDelegate>
...
@property (strong, nonatomic) WCStartPaymentParsedJsonData *sessionData;

@property (strong, nonatomic) WCViewFactory *viewFactory;

@end

@implementation WCJsonDialogViewController

- (void)viewDidLoad
{
    [super viewDidLoad];
    self.viewFactory = [[WCViewFactory alloc] init];
    self.placeholderText = NSLocalizedStringFromTable(@"JsonPlaceholder", kWCAppLocalizable, nil);
    ...
    self.dismissButton = [self.viewFactory buttonWithType:WCButtonTypeSecondary];
    self.dismissButton.translatesAutoresizingMaskIntoConstraints = NO;
    NSString *dismissButtonTitle = NSLocalizedStringFromTable(@"Dismiss", kWCAppLocalizable, nil);
    ...
    self.parseButton = [self.viewFactory buttonWithType:WCButtonTypePrimary];
    self.parseButton.translatesAutoresizingMaskIntoConstraints = NO;
    NSString *parseButtonTitle = NSLocalizedStringFromTable(@"Parse", kWCAppLocalizable, nil);
    ...
    self.dialogTitle.text = NSLocalizedStringFromTable(@"Paste", kWCAppLocalizable, nil);
    ...
    self.dialogMessage.text = NSLocalizedStringFromTable(@"JsonDialogMessage", kWCAppLocalizable, nil);
...
- (void)parsePressed
{
    ...
    WCStartPaymentParsedJsonData *data = self.sessionData;
...
- (void)parseJson:(NSString *)input {
    @try
    {
        self.sessionData = [[WCStartPaymentParsedJsonData alloc] initWithJSONString:input];
    }
...
- (BOOL)isValidSessionJson:(WCStartPaymentParsedJsonData *)data
{
...
{
    self.errorLabel.text = NSLocalizedStringFromTable(@"JsonErrorMessage", kWCAppLocalizable, nil);
}
